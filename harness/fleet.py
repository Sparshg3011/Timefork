...
def run_chaos(n_workers=4, n_jobs=40, kill_every=1.5, timeout=150.0, latency=None):
    ids = _enqueue_jobs(n_jobs)
    num = [0]
...
def main():
    ...
    if arg == "chaos":
        ...
        print(f"  -> {'zombie fenced out, counters stayed exactly 1' if ok else 'FAILED ' + str(z)}")
    else:
